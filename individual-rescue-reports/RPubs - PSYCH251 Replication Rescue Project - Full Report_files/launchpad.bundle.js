...
class ConsentCheck {
  /**
   * @param {Object[]} sectionData - Possible values: [{SectionData}, {SubSectionData}] | [{...SectionData, ...SubSectionData}]
  ...
  /**
   * Some CMPs push all properties to single Object, and others push 2 Objects inside an array. We will handle both cases.
   * @param {Object[]} sectionData - Possible values: [{SectionData}, {SubSectionData}] | [{...SectionData, ...SubSectionData}]
...
/**
 * @event CustomEvent#gppFullyLoaded - return CustomEvent object with property detail: GppConsentObject | detail: null
 ...
 * @fires CustomEvent#gppFullyLoaded
 * or
...
class GppVersion {
  ...
  awaitGppLibrary() {
    ...
    const interval = setInterval(() => {
      ++timesChecked;
      if (!this.flag_gppLibraryFullyLoaded && timesChecked === 10) {
        // fallback to ccpa
        clearInterval(interval);
        (0, _utils.dispatchCustomEvent)(`${this.moduleName}GppFullyLoaded`, null);
      }
      ...
      if (pingData && pingData.cmpStatus === 'loaded') {
        ...
        if (!this.isApiSupported(supportedAPIs)) {
          ...
          if (!this.flag_gppLibraryFullyLoaded) {
            ...
            (0, _utils.dispatchCustomEvent)(`${this.moduleName}GppFullyLoaded`, gppConsentObject);
          }
        }
  ...
  getVersionData(gppVersion, sectionPrefix, pingData) {
    ...
    if (gppVersion === _constants.GPP_VERSIONS.v1_0) {
      ...
      consentData = window.__gpp('getSection', null, sectionPrefix);
    }
...
class AtsHandler {
  ...
  checkConsentForAts = async (geoLocation, atsRules, integrations, notify) => {
    if (!this.cmpHandler.isUserPrivacySignalEnabled()) {
      this.iabConsent = await this.cmpHandler.iabCheckConsent();
...
class CmpHandler {
  ...
  checkTcfConsent = resolve => resolve(_iabConsentCheck.tcfConsentCheck.getTcfConsentObject(true, 'launchpad'));
  checkUsConsent = resolve => {
    window.addEventListener(`launchpadGppFullyLoaded`, event => {
      this.onGppLoadedConsentHandler(event.detail, resolve);
    });
    _iabConsentCheck.gppConsentCheck.getGppConsentObject(this.geoLocation.region, 'launchpad');
  };
  onGppLoadedConsentHandler = (consentObject, resolve) => {
    if (consentObject) {
...
function launchPadStart(configUpdates) {
  ...
  const defaultConfig = {"env":"production","configVersion":null,"logging":false,"cmpRules":[],"atsRules":[],"triggers":[],"preload":true,"libraryVersion":"1.7.4","geoTargetingUrl":"https://geo.privacymanager.io","ccpaConfigUrl":"https://ccpa-wrapper.privacymanager.io/ccpa","gdprConfigUrl":"https://gdpr-wrapper.privacymanager.io/gdpr","globalCmpConfigUrl":"https://cmp-wrapper.privacymanager.io","gppConfigUrl":"https://gpp-wrapper.privacymanager.io","purposeListLocation":"https://cmp-vendors.privacymanager.io/purpose-list.json","atsConfigUrl":"https://ats-wrapper.privacymanager.io","gdprBundleUrl":"https://gdpr.privacymanager.io/latest/gdpr.bundle.js","ccpaBundleUrl":"https://ccpa.privacymanager.io/latest/ccpa.bundle.js","globalCmpBundleUrl":"https://cmp.privacymanager.io/latest/cmp.bundle.js","gppBundleUrl":"https://gpp.privacymanager.io/latest/gpp.bundle.js"} || _liverampCmpUtils.config.generate();
