...
%DIF DEL manuscript_old.tex   Tue Sep  3 13:35:07 2024
%DIF ADD manuscript.tex       Mon Oct 14 16:45:33 2024
...
Imagine you are a graduate student \DIFdelbegin \DIFdel{, }\DIFdelend and you run a replication of a study that you are interested in building upon in your research.
The replication fails.
Perhaps the interaction you hoped for is directionally correct, but the point estimate is small and the confidence interval definitely \DIFdelbegin \DIFdel{overlaps }\DIFdelend \DIFaddbegin \DIFadd{contains }\DIFaddend 0, with a \(p\)-value of .3.
...
In psychology, \DIFdelbegin \DIFdel{large scale }\DIFdelend \DIFaddbegin \DIFadd{large-scale }\DIFaddend replication projects have found that around half of studies successfully replicate.
...
\DIFdelbegin \DIFdel{Large scale }\DIFdelend \DIFaddbegin \DIFadd{Large-scale }\DIFaddend replication projects in other disciplines have found 39/97 of studies with positive effects (40\%) replicating in cancer biology (\citeproc{ref-errington2021}{Errington et al., 2021}), 11/18 studies (61\%) replicating in economics (\citeproc{ref-camerer2016}{Camerer et al., 2016}), and 31/40 studies (78\%) replicating in experimental philosophy (\citeproc{ref-cova2021}{Cova et al., 2021}).
Across these disciplines, and especially in psychology, large-scale replications indicate \DIFdelbegin \DIFdel{a substantial chance of scientists encountering failed replications if they try }\DIFdelend \DIFaddbegin \DIFadd{that the probability of scientists failing }\DIFaddend to replicate published findings \DIFaddbegin \DIFadd{is substantial}\DIFaddend .

Replications can fail for many reasons\DIFdelbegin \DIFdel{, having }\DIFdelend \DIFaddbegin \DIFadd{. Some reasons for replication failure have }\DIFaddend to do with the original study\DIFdelbegin \DIFdel{or the replication .
Starting with the original study, one potential reason for replication failure }\DIFdelend \DIFaddbegin \DIFadd{, while others have to do with how the replication was conducted.
One potential reason }\DIFaddend is that the original result is fragile in some way.
...
Another possibility is that the original reported effect size \DIFdelbegin \DIFdel{might be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend inflated due to some combination of heterogeneity, \(p\)-hacking, low power, and publication bias.
\DIFdelbegin \DIFdel{A consequence of inflated effect sizes is that replication studies might }\DIFdelend \DIFaddbegin \DIFadd{If effect sizes are inflated, replication studies that }\DIFaddend select a sample size with appropriate statistical power for the reported effect size \DIFdelbegin \DIFdel{and then }\DIFdelend \DIFaddbegin \DIFadd{will }\DIFaddend have too small a sample to reliably detect the (true) smaller effect.
Replication studies might also end up underpowered due to unexpected attrition or noise in the replication sample.

\DIFdelbegin \DIFdel{Some other }\DIFdelend \DIFaddbegin \DIFadd{Other }\DIFaddend reasons for replication failure are primarily due to the \DIFaddbegin \DIFadd{design of the }\DIFaddend replication study.
A replication could observe different results from the original due to methodological differences.
These differences \DIFdelbegin \DIFdel{in design could be }\DIFdelend \DIFaddbegin \DIFadd{include }\DIFaddend intentional adaptations, unavoidable changes from lack of access to the original instructions or materials, or changes that \DIFdelbegin \DIFdel{were not expected }\DIFdelend \DIFaddbegin \DIFadd{the replicators did not expect }\DIFaddend to be critical.
A difference in the details of recruitment\DIFdelbegin \DIFdel{(e.g., online versus in-person) }\DIFdelend \DIFaddbegin \DIFadd{, such as changing from recruiting participants in person to online, }\DIFaddend might mean that particular details of a study implementation \DIFdelbegin \DIFdel{would no longer be }\DIFdelend \DIFaddbegin \DIFadd{are no longer }\DIFaddend appropriate, or the data quality controls \DIFdelbegin \DIFdel{and attention checks might be }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend inadequate.
If there are differences in time, place, or subject population between the two studies, \DIFdelbegin \DIFdel{corresponding changes to }\DIFdelend \DIFaddbegin \DIFadd{researchers may need to change }\DIFaddend the materials, instructions, or procedure \DIFdelbegin \DIFdel{could be needed to provide an adaptation of }\DIFdelend \DIFaddbegin \DIFadd{to adapt }\DIFaddend the experimental paradigm to the new context.
Such adaptation creates \DIFdelbegin \DIFdel{yet }\DIFdelend more challenges in interpretation: too many changes could cause differing results, but so could \DIFdelbegin \DIFdel{not enough }\DIFdelend \DIFaddbegin \DIFadd{too few }\DIFaddend changes.
In sum, when replications fail, we generally do not know why they failed, although we may speculate.

For the goals of large-scale replication projects, it does not matter why any particular study failed to replicate, because the aim is to estimate the proportion of \DIFdelbegin \DIFdel{replication effects }\DIFdelend \DIFaddbegin \DIFadd{effects that replicate }\DIFaddend in a literature.
In contrast, \DIFdelbegin \DIFdel{however, }\DIFdelend when individual scientists attempt to replicate an effect with the goal of building on it, they \DIFdelbegin \DIFdel{do care about }\DIFdelend \DIFaddbegin \DIFadd{care }\DIFaddend why an individual replication failed.
...
For those individual scientists, it is much more important \DIFaddbegin \DIFadd{to know }\DIFaddend whether the replication failure is due to a fixable issue with \DIFdelbegin \DIFdel{the replication }\DIFdelend \DIFaddbegin \DIFadd{their replication attempt }\DIFaddend or an intrinsic feature of the original result.

\DIFdelbegin \DIFdel{Re-replication can serve as a way to test explanations for replication failure.
}\DIFdelend While it is possible to speculate after the fact about potential causes of a single replication failure, in most circumstances these causes are hard to discern (absent clear evidence of errors).
\DIFdelbegin \DIFdel{In contrast, a second replication provides a chance }\DIFdelend \DIFaddbegin \DIFadd{Re-replication can serve as a way }\DIFaddend to test explanations \DIFdelbegin \DIFdel{by making modifications -- as well as giving }\DIFdelend \DIFaddbegin \DIFadd{for replication failure.
Re-replications can make modifications to the first replication and give }\DIFaddend another chance for researchers to measure an effect of interest.
...
The re-replication success rate was thus 2/9 (22\%) for studies with non-significant first replications \DIFdelbegin \DIFdel{, }\DIFdelend under favorable, high-resource circumstances.

While Ebersole et al. (\citeproc{ref-ebersole2020}{2020}) looked at re-replication success rate in a \DIFdelbegin \DIFdel{high resource }\DIFdelend \DIFaddbegin \DIFadd{high-resource }\DIFaddend setting with access to large, multi-site samples and expert input, most \DIFdelbegin \DIFdel{early career }\DIFdelend \DIFaddbegin \DIFadd{early-career }\DIFaddend scientists do not have \DIFdelbegin \DIFdel{those kinds of }\DIFdelend \DIFaddbegin \DIFadd{comparable }\DIFaddend resources to devote to re-replication attempts.
On the other hand, \DIFdelbegin \DIFdel{early career }\DIFdelend \DIFaddbegin \DIFadd{early-career }\DIFaddend researchers may also be prone to a different distribution of causes of initial replication failure.
Thus, it is an open question what the re-replication rate is for failed replications by \DIFdelbegin \DIFdel{early career }\DIFdelend \DIFaddbegin \DIFadd{early-career }\DIFaddend researchers.
Further, estimating this rate could inform researchers' decisions on what to do next \DIFdelbegin \DIFdel{after failed replications}\DIFdelend \DIFaddbegin \DIFadd{when a replication fails}\DIFaddend .
...
In the present paper, we report the results of 17 \DIFdelbegin \DIFdel{re-replication }\DIFdelend rescue projects.
...
In previous years, students \DIFdelbegin \DIFdel{have }\DIFdelend conducted replication projects (\citeproc{ref-boyce2023}{Boyce et al., 2023}; \citeproc{ref-hawkins2018}{Hawkins et al., 2018}).
...
We report on the result of \DIFaddbegin \DIFadd{the }\DIFaddend 17 rescue projects \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{by students who completed data collection and }\DIFaddend opted to be \DIFaddbegin \DIFadd{a }\DIFaddend part of the paper\DIFdelbegin \DIFdel{and completed data collection}\DIFdelend .

A spreadsheet of projects, individual project write-ups (both first replications and rescues), individual project data and analyses for rescue projects, and the analytic code for this paper are all available at \DIFdelbegin %DIFDELCMD < \url{https://osf.io/cyk5w/}%%%
...
Students were required to think of reasons \DIFaddbegin \DIFadd{why }\DIFaddend the first replication might not have worked, and address them if they could.
A list of possible reasons and solutions was given to students (\url{https://osf.io/dfwk2}).
In general, we encouraged students to add manipulation checks as appropriate \DIFdelbegin \DIFdel{, }\DIFdelend and to adapt their materials to online studies, which may benefit from attention checks and/or shorter duration.
For instance, the rescue of Paxton et al. (\citeproc{ref-paxton2012}{2012}) switched from using the Cognitive Reflection Test (CRT) which many online participants are over-familiar with to using the newer CRT-2 which is \DIFdelbegin \DIFdel{less overused }\DIFdelend \DIFaddbegin \DIFadd{used less }\DIFaddend but attempts to measure the same construct (\citeproc{ref-thomson2016}{Thomson \& Oppenheimer, 2016}).
...
The replication ported this experiment online and had participants \DIFdelbegin \DIFdel{click a drop down }\DIFdelend \DIFaddbegin \DIFadd{use a dropdown }\DIFaddend menu to select left or right for each turn.
The rescue removed the unwieldy \DIFdelbegin \DIFdel{drop down }\DIFdelend \DIFaddbegin \DIFadd{dropdown }\DIFaddend menus and opted to have participants press keys on the keyboard to indicate the direction of a turn, which seemed like a more natural interface.
...
With one exception, samples were collected on Prolific (the rescue of \citeproc{ref-yeshurun2003}{Yeshurun \& Levy, 2003} ran \DIFdelbegin \DIFdel{in-person }\DIFdelend \DIFaddbegin \DIFadd{in person }\DIFaddend on the Stanford student subject pool).
We tried to power studies adequately (with a target \DIFaddbegin \DIFadd{sample size }\DIFaddend of 2.5x \DIFdelbegin \DIFdel{original }\DIFdelend \DIFaddbegin \DIFadd{the original study }\DIFaddend following \citeproc{ref-simonsohn2015}{Simonsohn, 2015}), but due to cost constraints, not all studies were powered at this level.
...
When effects were not reported in consistent ways across original and replications, we recalculated effects from raw data \DIFdelbegin \DIFdel{when necessary }\DIFdelend to obtain comparable values.
...
These potential correlates included the subfield of the study (cognitive, social, or other psychology), its publication year, experimental design features including whether it \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend a within- or between-subject design, whether each condition was instantiated with one vignette or multiple, how many items each participant saw, and whether there were open materials and open data.

For the original study and each replication, we recorded the number of participants \DIFdelbegin \DIFdel{post-exclusions}\DIFdelend \DIFaddbegin \DIFadd{after exclusions}\DIFaddend .
For studies where \DIFdelbegin \DIFdel{some }\DIFdelend extra conditions were dropped in some replications, we counted only the participants in the key conditions all replications had for comparability.
...
We also recorded whether each study was conducted \DIFdelbegin \DIFdel{online using a crowd-sourced }\DIFdelend \DIFaddbegin \DIFadd{using an online crowdsourcing }\DIFaddend platform or not.

\section{Results}\label{results}

We first discuss the overall rate of replication in the rescue projects, \DIFdelbegin \DIFdel{followed by looking }\DIFdelend \DIFaddbegin \DIFadd{then we look }\DIFaddend at the relative effect sizes of the rescues and correlates of rescue success.
...
\begin{figure}
\includegraphics[width=1\linewidth]{manuscript_files/figure-latex/smd-1} \caption{Standardized effect sizes of original studies, first replications, rescues, and additional replications if available. Due to the large effect size of \DIFdelbeginFL \DIFdelFL{a couple }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{some }\DIFaddendFL studies, \DIFdelbeginFL \DIFdelFL{large effect }\DIFdelendFL studies \DIFaddbeginFL \DIFaddFL{with large effects }\DIFaddendFL are shown in a separate panel. In a few cases, the first replication's key effect was in the same direction as the original with a 95\% CI that did not \DIFdelbeginFL \DIFdelFL{overlap 0; however}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{contain 0. However}\DIFaddendFL , in these cases the larger pattern of results was not consistent between original and first replication.}\label{fig:smd}
\end{figure}
...
Though we did not conduct a systematic review of the literature to identify all possible replications, we also \DIFdelbegin \DIFdel{consider }\DIFdelend \DIFaddbegin \DIFadd{considered }\DIFaddend the effect sizes obtained in additional replications when we were aware of them (either from other class projects, or external replications in the literature).
...
On the other hand, one study may find a statistically significant, but imprecisely estimated\DIFaddbegin \DIFadd{, }\DIFaddend effect in a small sample, and a second study may find a near-zero (null) effect\DIFdelbegin \DIFdel{, but the }\DIFdelend \DIFaddbegin \DIFadd{. The }\DIFaddend effect estimates from the two studies \DIFdelbegin \DIFdel{may }\DIFdelend \DIFaddbegin \DIFadd{might }\DIFaddend be statistically compatible, despite one supporting a hypothesized difference and the other not.
...
That is, \(p\)-original \DIFdelbegin \DIFdel{is a measure of }\DIFdelend \DIFaddbegin \DIFadd{measures }\DIFaddend the statistical consistency of the original result with the totality of the replications, assuming a given level of heterogeneity of effects between studies.
...
As secondary measures, we calculated the \(p\)-original values between a) the original and \DIFdelbegin \DIFdel{just }\DIFdelend the first replication and rescue (i.e.~excluding additional replications), b) the original and the rescue, c) the original and non-rescue replications, and d) the rescue and other replications.
...
For c) the original versus all the non-rescue replications, the median value of \(p\)-original was 0.14 {[}IQR: 0 - 0.38{]}, and 35\% of the \(p\)-original values were less than .05.
...
\caption{\label{tab:cor-table}Correlations between \DIFdelbeginFL \DIFdelFL{an }\DIFdelendFL individual \DIFdelbeginFL \DIFdelFL{predictor }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{predictors }\DIFaddendFL and the subjective replication score of the rescue project. The first set of predictors were pre-registered based on the correlates used in Boyce et al. (2023). Original authors at Stanford refers to \DIFaddbeginFL \DIFaddFL{whether the any of the study's authors were affiliated with Stanford }\DIFaddendFL at the time of the first replication\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL as this may mean the first replication was not independent. For details about how the predictors were coded, see the Methods section of Boyce et al. (2023). The last three predictors were added post-hoc.}
\centering
\begin{tabular}[t]{rcc}
\toprule
Predictors & r & p\\
...
The number of rescues is small, and many of these predictors are correlated, so we caution against \DIFdelbegin \DIFdel{over interpretation}\DIFdelend \DIFaddbegin \DIFadd{over-interpretation}\DIFaddend .

The strongest correlates of rescue success were open materials, a small sample size \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the first replication, a small sample size \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the first replication relative to the original sample size, and a large rescue sample size relative to the first replication.
...
Small replication samples relative to original and rescue could be due to both a) powering a replication according to a reported large effect size or b) difficulties with recruitment or high exclusion rates leading to a \DIFaddbegin \DIFadd{sample size }\DIFaddend smaller than intended\DIFdelbegin \DIFdel{sample}\DIFdelend .
...
This study looked at the influence of guided thinking on whether \DIFdelbegin \DIFdel{or not }\DIFdelend people gave correct justifications (drawn or written) for their answer on the Monty Hall problem.
The original paper reported correct \DIFdelbegin \DIFdel{justification }\DIFdelend \DIFaddbegin \DIFadd{justifications }\DIFaddend from 2/67 (3\%) \DIFaddbegin \DIFadd{participants }\DIFaddend in the control condition and 13/34 (38\%) in the guided thinking condition.
...
\DIFdelbegin \DIFdel{Collectively, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend believe these changes \DIFaddbegin \DIFadd{collectively }\DIFaddend brought the rescue closer to the intent of the original.
...
The first replication struggled to recruit participants to complete a followup session after sleep and \DIFdelbegin \DIFdel{only }\DIFdelend reported a sample size of \DIFaddbegin \DIFadd{only }\DIFaddend 23 (the original had 48).
The rescue attempted to recruit a larger sample (target 88), but \DIFdelbegin \DIFdel{also }\DIFdelend experienced difficulties getting participants to complete the second part of the experiment 12 hours after the first, \DIFdelbegin \DIFdel{managing also only to recruit }\DIFdelend \DIFaddbegin \DIFadd{also managing to recruit only }\DIFaddend 23 people.
...
Denis et al. (\citeproc{ref-denis2022}{2022})\DIFaddbegin \DIFadd{, }\DIFaddend which has an overlapping author team to Payne et al. (\citeproc{ref-payne2008}{2008})\DIFaddbegin \DIFadd{, }\DIFaddend is included here as an additional replication.
...
It is reasonable to expect online studies of reading time to be noisier than in-person studies because the environments participants completed the study in \DIFdelbegin \DIFdel{varied }\DIFdelend \DIFaddbegin \DIFadd{vary }\DIFaddend much more online.
...
Additionally, they were chosen non-randomly, as they were doubly selected for based on student \DIFdelbegin \DIFdel{interest -- once }\DIFdelend \DIFaddbegin \DIFadd{interest---once }\DIFaddend for the original replication and again for the rescue.
However, this selection bias is likely to correlate with how graduate students and other \DIFdelbegin \DIFdel{early career }\DIFdelend \DIFaddbegin \DIFadd{early-career }\DIFaddend researchers choose what topics to work on and what studies to build on.
...
One potentially salient limitation of our work here is that the rescue projects (and the first replications) were primarily conducted by \DIFdelbegin \DIFdel{first year }\DIFdelend \DIFaddbegin \DIFadd{first-year }\DIFaddend PhD students; hence the generality of our conclusions to other investigators is unknown. We believe, however, that this is more of an apparent than an actual limitation. Students were enrolled in a \DIFdelbegin \DIFdel{highly-competitive }\DIFdelend \DIFaddbegin \DIFadd{highly competitive }\DIFaddend graduate program where it is expected that PhD students author peer-reviewed papers\DIFdelbegin \DIFdel{; several }\DIFdelend \DIFaddbegin \DIFadd{. Several }\DIFaddend of the rescuers \DIFdelbegin \DIFdel{already had }\DIFdelend \DIFaddbegin \DIFadd{had already }\DIFaddend published first-authored peer reviewed articles. Thus, while we refer to the rescuers as students, they are investigators doing experiments that become part of the literature.
