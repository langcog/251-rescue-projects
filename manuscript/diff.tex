...
%DIF DEL manuscript_submitted.tex   Mon Jun 10 15:16:14 2024
%DIF ADD manuscript.tex             Tue Jul 30 10:41:44 2024
...
%DIF 200a200-205
\MyNewLabel{ADDRTXT}{Psych}{Department of Psychology, Stanford University} %DIF > 
\MyNewLabel{ADDRTXT}{GSB}{Graduate School of Business, Stanford University} %DIF > 
\MyNewLabel{ADDRTXT}{Edu}{Graduate School of Education, Stanford University} %DIF > 
\MyNewLabel{ADDRTXT}{Symsys}{Symbolic Systems Program, Stanford University} %DIF > 
\MyNewLabel{ADDRTXT}{Philosophy}{Department of Philosophy, Stanford University} %DIF > 
\MyNewLabel{ADDRTXT}{CS}{Department of Computer Science, Stanford University} %DIF > 
%DIF -------

% Create labels for Footnotes if they are given by code
%DIF 202a208
\MyNewLabel{ANOTETXT}{corresp}{Corresponding author. Email: \href{mailto:vboyce@stanford.edu}{\nolinkurl{vboyce@stanford.edu}}} %DIF > 
%DIF -------
...
%DIF 254a261
\createaddrlblbycode{Psych} %DIF > 
%DIF -------


%DIF 256a264
\createanotelblbycode{corresp} %DIF > 
%DIF -------
...
%DIF < ]{anonymous}
%DIF -------
\addrlblbycode{Psych}% %DIF > 
,% %DIF > 
$\anotelblbycode{corresp}$% %DIF > 
]{Veronica Boyce} %DIF > 
 %DIF > 
\addrbycode{Psych} %DIF > 
 %DIF > 
 %DIF > 
\createaddrlblbycode{Psych} %DIF > 
 %DIF > 
...
\addrbycode{Psych} %DIF > 
 %DIF > 
 %DIF > 
\createaddrlblbycode{GSB} %DIF > 
 %DIF > 
 %DIF > 
 %DIF > 
\author[% %DIF > 
\addrlblbycode{GSB}% %DIF > 
]{Ziwen Chen} %DIF > 
 %DIF > 
\addrbycode{GSB} %DIF > 
 %DIF > 
 %DIF > 
\createaddrlblbycode{Edu} %DIF > 
 %DIF > 
 %DIF > 
 %DIF > 
\author[% %DIF > 
\addrlblbycode{Edu}% %DIF > 
]{Howard Chiu} %DIF > 
 %DIF > 
\addrbycode{Edu} %DIF > 
 %DIF > 
...
\addrbycode{Psych} %DIF > 
 %DIF > 
 %DIF > 
\createaddrlblbycode{Symsys} %DIF > 
 %DIF > 
 %DIF > 
 %DIF > 
\author[% %DIF > 
\addrlblbycode{Symsys}% %DIF > 
]{Chuqi Hu} %DIF > 
 %DIF > 
\addrbycode{Symsys} %DIF > 
 %DIF > 
 %DIF > 
\createaddrlblbycode{Philosophy} %DIF > 
 %DIF > 
 %DIF > 
 %DIF > 
\author[% %DIF > 
\addrlblbycode{Philosophy}% %DIF > 
]{Bendix Kemmann} %DIF > 
 %DIF > 
\addrbycode{Philosophy} %DIF > 
 %DIF > 
 %DIF > 
\createaddrlblbycode{Psych} %DIF > 
 %DIF > 
...
,% %DIF > 
\addrlblbycode{Philosophy}% %DIF > 
]{Alexander Pereira} %DIF > 
 %DIF > 
\addrbycode{Psych} %DIF > 
\addrbycode{Philosophy} %DIF > 
 %DIF > 
 %DIF > 
\createaddrlblbycode{CS} %DIF > 
 %DIF > 
 %DIF > 
 %DIF > 
\author[% %DIF > 
\addrlblbycode{CS}% %DIF > 
]{Jordan Troutman} %DIF > 
 %DIF > 
\addrbycode{CS} %DIF > 
 %DIF > 
...
Across 100 studies with positive results, the reproducibility project in psychology (RP:P) replicated 36\%-47\% depending on the metric for replication success (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-openscienceconsortium}{Open Science Consortium, 2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-opensciencecollaboration}{Open Science Collaboration, 2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ).
...
One estimate comes from Ebersole et al. (\citeproc{ref-ebersole2020}{2020}). After the original RP:P study reported of a replication rate around 40\% (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-openscienceconsortium}{Open Science Consortium, 2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-opensciencecollaboration}{Open Science Collaboration, 2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ), Gilbert et al. (\citeproc{ref-gilbert2016}{2016}) raised concerns that methodological differences might explain the low replication rate. In 11 of the RP:P studies, the original authors had raised concerns about protocol fidelity prior to data collection.
Of these 11 with concerns, 10 failed to obtain significant results in the RP:P replication.
Thus, Ebersole et al. (\citeproc{ref-ebersole2020}{2020}) re-replicated 10 of these 11 in larger samples, using both the RP:P protocol and a new protocol revised under advice of the original authors or other experts. This study thus tested whether larger sample sizes and/or ``better'' methodologies would ``rescue'' the failed RP:P replications.
...
While Ebersole et al. (\citeproc{ref-ebersole2020}{2020}) looked at re-replication success rate in a high resource setting with access to large, multi-site samples and expert input, most early career scientists do not have those kinds of resources to devote to re-replication attempts. On the other hand, early career researchers may also be prone to a different distribution of causes of initial replication failure. Thus, it is an open question what the re-replication rate is for failed replications by early career researchers. Further, estimating this rate could inform researchers' decisions on what to do next after failed replications.
...
In the present paper, we report the results of 17 re-replication rescue projects. We find that a minority of projects (5/17, 29\%) successfully recovered the original effect.

\section{Methods}\label{methods}

\DIFdelbegin %DIFDELCMD < {[}%%%
\DIFdel{class name}%DIFDELCMD < {]} %%%
\DIFdelend \DIFaddbegin \DIFadd{PSYCH 251 }\DIFaddend is a graduate-level experimental methods class taught by \DIFdelbegin %DIFDELCMD < {[}%%%
\DIFdel{anonymous}%DIFDELCMD < {]}%%%
...
A spreadsheet of projects, individual project write-ups (both first replications and rescues), individual project data and analyses for rescue projects, and the analytic code for this paper are all available at \DIFdelbegin %DIFDELCMD < \url{https://osf.io/cyk5w/?view_only=620212fb6b55490d9ae3a555e7b58eca}%%%
...
For instance, the rescue of Paxton et al. (\citeproc{ref-paxton2012}{2012}) switched from using the cognitive reflection task (CRT) which many online participants are over-familiar with to using the newer CRT-2 which is less overused but attempts to measure the same construct (\citeproc{ref-thomson2016}{Thomson \& Oppenheimer, 2016}).
...
With one exception, samples were collected on Prolific (the rescue of \citeproc{ref-yeshurun2003}{Yeshurun \& Levy, 2003} ran in-person on the \DIFdelbegin %DIFDELCMD < {[}%%%
\DIFdel{anonymized university}%DIFDELCMD < {]} %%%
...
Each project was rated on the basis of subjective replication success (on a 0--1 scale) by both \DIFdelbegin %DIFDELCMD < {[}%%%
\DIFdel{anonymous}%DIFDELCMD < {]} %%%
\DIFdelend \DIFaddbegin \DIFadd{MCF }\DIFaddend and one of \DIFdelbegin %DIFDELCMD < {[}%%%
\DIFdel{also anonymous}%DIFDELCMD < {]}%%%
...
\DIFaddbegin \DIFadd{We first discuss the overall rate of replication in the rescue projects, followed by looking at the relative effect sizes of the rescues and correlates of rescue success. In the last subsection we describe the topic of each studies and focus on a few interesting case studies.
}

\DIFaddend \subsection{Overall replication rate}\label{overall-replication-rate}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_anon_files/figure-latex/smd-1} %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_files/figure-latex/smd-1} \DIFaddendFL \caption{Standardized effect sizes of original studies, first replications, rescues, and additional replications if available. Due to the large effect size of a couple studies, large effect studies are shown in a separate panel. In a few cases, the first replication's key effect was \DIFdelbeginFL \DIFdelFL{non-zero }\DIFdelendFL in the same direction as the original \DIFaddbeginFL \DIFaddFL{with a 95\% CI that did not overlap 0}\DIFaddendFL ; however, in these cases the larger pattern of results was not fully consistent between original and first replication.  }\label{fig:smd}
\end{figure}
...
As secondary measures, we calculated the \(p\)-original values between a) the original and \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{just the first replication and }\DIFaddend rescue, b) the original and \DIFaddbegin \DIFadd{the rescue, c) the original and }\DIFaddend non-rescue replications, and \DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend ) the rescue and other replications.
For a) the original versus the \DIFaddbegin \DIFadd{first replication and the rescue, the median value of \(p\)-original was 0.1 }{[}\DIFadd{IQR: 0.01 - 0.27}{]}\DIFadd{, and 35\% of the \(p\)-original values were less than .05.
For b) the original versus the }\DIFaddend rescue, the median value of \(p\)-original was 0.06 {[}IQR: 0.01 - 0.52{]}, and 47\% of the \(p\)-original values were less than .05.
For \DIFdelbegin \DIFdel{b}\DIFdelend \DIFaddbegin \DIFadd{c}\DIFaddend ) the original versus all the non-rescue replications, the median value of \(p\)-original was 0.14 {[}IQR: 0.01 - 0.38{]}, and 35\% of the \(p\)-original values were less than .05.
For \DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{d}\DIFaddend ) the rescue versus the other replications, the median value of \(p\)-original was 0.31 {[}IQR: 0.14 - 0.78{]}, and 24\% of the \(p\)-original values were less than .05.
...
\caption{\label{tab:cor-table}Correlations between an individual predictor and the subjective replication score of the rescue project. The first set of predictors were pre-registered based on the correlates used in Boyce et al. (2023). \DIFaddbeginFL \DIFaddFL{Original authors at Stanford refers to at the time of the first replication as this may mean the first replication was not independent. For details about how the predictors were coded, see the Methods section of Boyce et al. (2023). }\DIFaddendFL The last three predictors were added post-hoc.}
\centering
\begin{tabular}[t]{rcc}
\toprule
Predictors & r & p\\
...
\DIFaddbeginFL \DIFaddFL{Original authors at }\DIFaddendFL Stanford & -0.233 & 0.369\\
Log \DIFaddbeginFL \DIFaddFL{number of }\DIFaddendFL trials & 0.005 & 0.985\\
...
Are there signals in the features of original or replication studies that predict whether a re-replication will succeed? To address this question, we report correlations between the set of predictor variables used in Boyce et al. (\citeproc{ref-boyce2023}{2023}) and the subjective replication scores of the rescues.
...
Given the mix of successful and unsuccessful rescue projects, we \DIFdelbegin \DIFdel{discuss }\DIFdelend \DIFaddbegin \DIFadd{briefly discuss all the projects and go more in depth on }\DIFaddend a few projects where we have speculations about why they turned out the way they did.
...
Ngo et al. (\citeproc{ref-ngo2019}{2019}) was a second successful rescue. The original study found a large effect; hence the first replication, powering for 80\% power on the reported effect, recruited a small sample of 12 people. This small study failed to find the original effect.
The rescue, powered using 2.5x the original sample (as recommended by \citeproc{ref-simonsohn2015}{Simonsohn, 2015}), recovered a clear effect (albeit a much smaller one). \DIFaddbegin \DIFadd{An additional replication from the literature, Ngo \& Newcombe (\mbox{%DIFAUXCMD
\citeproc{ref-ngo2021}{2021}}\hspace{0pt}%DIFAUXCMD
) successfully replicated Ngo et al. (\mbox{%DIFAUXCMD
\citeproc{ref-ngo2019}{2019}}\hspace{0pt}%DIFAUXCMD
).
}\DIFaddend There are reasons to think that some effect sizes in the literature may be inflated (\citeproc{ref-camerer2018}{Camerer et al., 2018}; \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citeproc{ref-openscienceconsortium}{Open Science Consortium, 2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citeproc{ref-opensciencecollaboration}{Open Science Collaboration, 2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ), and it is also possible that changes to experiments or switches to online could result in noisier samples and thus smaller effect sizes.
...
Payne et al. (\citeproc{ref-payne2008}{2008}) was a study of the effects of sleep on memory consolidation. They showed participants a number of images and then hours later (after either sleep or no sleep) measured their recall for parts of the images.
...
However, an online replication by Denis et al. (\citeproc{ref-denis2022}{2022}) reports qualitatively similar (but quantitatively smaller) results to Payne et al. (\citeproc{ref-payne2008}{2008}) on related but not identical measures. \DIFaddbegin \DIFadd{Denis et al. (\mbox{%DIFAUXCMD
\citeproc{ref-denis2022}{2022}}\hspace{0pt}%DIFAUXCMD
) which has an overlapping author team to Payne et al. (\mbox{%DIFAUXCMD
\citeproc{ref-payne2008}{2008}}\hspace{0pt}%DIFAUXCMD
) is included here as an additional replication.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Birch \& Bloom (\mbox{%DIFAUXCMD
\citeproc{ref-birch2007}{2007}}\hspace{0pt}%DIFAUXCMD
) is about a false belief task comparing ignorance and plausible knowledge conditions. The first replication, an additional class replication, and the rescue all found effects directionally consistent with the original, but much smaller.
}

\DIFadd{Craig \& Richeson (\mbox{%DIFAUXCMD
\citeproc{ref-craig2014}{2014}}\hspace{0pt}%DIFAUXCMD
) had White Americans read an article about shifting racial demographics in the US, either with or without a framing that assuaged the potential threat, and found that reading the un-assuaged version led to the endorsement of more conservative positions.
}

\DIFadd{Gong et al. (\mbox{%DIFAUXCMD
\citeproc{ref-gong2019}{2019}}\hspace{0pt}%DIFAUXCMD
) looked at the interplay between age and willingness to donate in older adults in Hong Kong and found a three-way interaction between age, type of donation, and relationship to recipient. Neither the replication that sampled US adults nor the rescue that sampled East Asian adults recovered this interaction.
}

\DIFadd{Haimovitz \& Dweck (\mbox{%DIFAUXCMD
\citeproc{ref-haimovitz2016}{2016}}\hspace{0pt}%DIFAUXCMD
) manipulated parents' mindsets with ``failure-is-enhancing'' or ``failure-is-debilitating'' primes and looked at their reactions to a hypothetical situation in which a child received a failing grade. The first replication was directionally consistent with the original, but an additional class replication was not. The rescue added manipulation checks which passed, but did not recover the target effect.
}

\DIFadd{Hopkins et al. (\mbox{%DIFAUXCMD
\citeproc{ref-hopkins2016}{2016}}\hspace{0pt}%DIFAUXCMD
) examined whether people prefer reductive explanations to horizontal explanations. While the original found a preference for reductive explanations, the replication and rescue both did not.
}

\DIFadd{Jara-Ettinger et al. (\mbox{%DIFAUXCMD
\citeproc{ref-jara-ettinger2022}{2022}}\hspace{0pt}%DIFAUXCMD
) looked at shape bias in US adults across 3 shape exemplars and found a preference to extend nouns on the basis of shape over color or material. The replication and rescue both found an above chance preference for shape, but with widely different item-level effects and relative preferences for color and material.
}

\DIFadd{Paxton et al. (\mbox{%DIFAUXCMD
\citeproc{ref-paxton2012}{2012}}\hspace{0pt}%DIFAUXCMD
) found that participants who completed the CRT before the moral judgement task gave more utilitarian answers than those who did the moral judgment task before the CRT. Neither the replication nor the rescue found this effect.
}

\DIFadd{Porter et al. (\mbox{%DIFAUXCMD
\citeproc{ref-porter2016}{2016}}\hspace{0pt}%DIFAUXCMD
) looked at how linguistic intergroup bias influences judgments of shared group membership. The replication was directionally against the original, the rescue found that effect, although the manipulation check failed.
Schechtman et al. (\mbox{%DIFAUXCMD
\citeproc{ref-schechtman2010}{2010}}\hspace{0pt}%DIFAUXCMD
) looked at how widely people generalize tones that previously have been paired with losses or gains, and found that the loss-paired tones were generalized more. The first replication and rescue both found the same main effect, but with a very different pattern over distance.
}

\DIFadd{As discussed in methods, Tarampi et al. (\mbox{%DIFAUXCMD
\citeproc{ref-tarampi2016}{2016}}\hspace{0pt}%DIFAUXCMD
) had participants do a timed navigation task and found an interaction between participant gender and the framing of the task (social or spatial) on performance. Neither first replication nor rescue recovered the interaction.
Todd et al. (\mbox{%DIFAUXCMD
\citeproc{ref-todd2016}{2016}}\hspace{0pt}%DIFAUXCMD
) looked at participants speed at classifying toys and guns as a function of whether they had just need a White or Black boy's face. Todd et al. (\mbox{%DIFAUXCMD
\citeproc{ref-todd2016}{2016}}\hspace{0pt}%DIFAUXCMD
) found an interaction where Black faces primed faster ``gun'' responses and White faces faster ``toy'' responses. When we reached out to the original authors, they offered to run a parallel rescue on their student subject pool, which we have here as an additional replication. The first replication did not find the interaction, but both the better-powered rescue did.
}

\DIFadd{Yeshurun \& Levy (\mbox{%DIFAUXCMD
\citeproc{ref-yeshurun2003}{2003}}\hspace{0pt}%DIFAUXCMD
) looked at the relationship between spatial attention and temporal resolution. They measured the threshold where two flashes of light are percieved as one flash versus two as a function of whether the location of the flashes was pre-cued or not. Yeshurun \& Levy (\mbox{%DIFAUXCMD
\citeproc{ref-yeshurun2003}{2003}}\hspace{0pt}%DIFAUXCMD
) found that cueing led to less temporal resolution. This was independently replicated by Chica \& Christie (\mbox{%DIFAUXCMD
\citeproc{ref-chica2009}{2009}}\hspace{0pt}%DIFAUXCMD
), but was not replicated by either the student replication or the rescue.
}
...
\DIFdelbegin \DIFdel{These diminished and inconsistent effects suggest that even if a re-replication ``works'', it may be difficult to build upon as follow-up studies will need large samples to detect small effects.
}\DIFdelend \DIFaddbegin 

\subsection{\DIFadd{Limitations}}\label{limitations}
\DIFaddend 

The reported rescue projects are a small sample of replications, leading to highly uncertain estimates.
\DIFdelbegin \DIFdel{They are also }\DIFdelend \DIFaddbegin \DIFadd{Additionally, they were }\DIFaddend chosen non-randomly, as they \DIFdelbegin \DIFdel{have been selected for twice }\DIFdelend \DIFaddbegin \DIFadd{were doubly selected for }\DIFaddend based on student interest -- once for the original replication and again for the rescue.
...
These limitations are representative of the sort of resource limitations often faced by early-career researchers. That said, it is possible that different results might be obtained in better-resourced settings, by scientists with more expertise and more time.

\DIFaddbegin \DIFadd{Due to the limited number of replications that are conducted and systematically reported, and the even smaller number of re-replications reported, there is a lot of uncertainty in how much replicability varies across features of the original experiment and replications.}\\
\DIFadd{\#\# Conclusion
...
\DIFadd{Replications are done for many reasons, and researchers often have some prior beliefs about whether a replication will succeed. These beliefs may influence how a researcher interprets a failed replication and thus whether they think it's worth trying to rescue. For instance, a researcher who has high credence in the main result is more likely to attribute failure to specific details in the replication and thus want to rescue by tweaking these details. On the other side, a researcher who is dubious of the methods, results, or statistical approaches, or who has experienced failures to replicate and extend the work, may be quicker to interpret a failure as a more definitive negative result. Interpretations may also depend on the perceived strength of the replication and what effect size is cared about.
}

\DIFadd{While we focus here on a narrow type of replication and rescue, that done by an early career researcher with the goal of establishing whether this is a result }\emph{\DIFadd{they}} \DIFadd{can build on, it exists as part of a wider ecosystem of replications that are done for different reasons with different properties.
}

\DIFaddend We opened with a question about what an early-career psychology researcher should do given a failed replication: should they try again or give up and move on?
...
\DIFdelbegin \section*{\DIFdel{Contributions}}%DIFAUXCMD
%DIFDELCMD < \label{contributions}
%DIFDELCMD < %%%
\addcontentsline{toc}{section}{\DIFdel{Contributions}}
%DIFAUXCMD
%DIFDELCMD < 
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Contributed to conception and design: VB and MCF
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Contributed to acquisition of data: all authors except VB, BP, and MCF
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Contributed to analysis and interpretation of data: all authors
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Drafted and/or revised the article: VB, BP, and MCF
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{Approved the submitted version for publication: all authors
}
...
%DIFDELCMD < %%%
\DIFdelend \section*{Acknowledgements}\label{acknowledgements}
\addcontentsline{toc}{section}{Acknowledgements}

We thank the students from previous years of PSYCH 251 who graciously agreed to let us re-replicate their projects and shared their work with us. We thank the other members of the PSYCH 251 teaching team and students for feedback on projects. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\section*{\DIFdel{Funding}}%DIFAUXCMD
%DIFDELCMD < \label{funding}
%DIFDELCMD < %%%
\addcontentsline{toc}{section}{\DIFdel{Funding}}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Data collection was supported by funding from the Stanford Departments of Psychology and Philosophy, Graduate School of Education, and the Symbolic Systems program.

\section*{\DIFdelbegin \DIFdel{Competing Interests}\DIFdelend \DIFaddbegin \DIFadd{Author Contributions}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{competing-interests}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{author-contributions}
\DIFaddend \addcontentsline{toc}{section}{\DIFdelbegin \DIFdel{Competing Interests}\DIFdelend \DIFaddbegin \DIFadd{Author Contributions}\DIFaddend }

\DIFdelbegin \DIFdel{The authors declare no competing interests}\DIFdelend \DIFaddbegin \DIFadd{VB and MCF conceptualized the study. BP contributed project administration, VB contributed formal analysis, and MCF contributed supervision}\DIFaddend .

\DIFdelbegin \section*{\DIFdel{Data accessibility}}%DIFAUXCMD
%DIFDELCMD < \label{data-accessibility}
%DIFDELCMD < %%%
\addcontentsline{toc}{section}{\DIFdel{Data accessibility}}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{A spreadsheet of projects, individual project write-ups (both first replications and rescues), individual project data and analyses for rescue projects , }\DIFdelend \DIFaddbegin \DIFadd{Authors other than VB, BP, }\DIFaddend and \DIFdelbegin \DIFdel{the analytic code for this paper are all available at }%DIFDELCMD < \url{https://osf.io/cyk5w}%%%
\DIFdelend \DIFaddbegin \DIFadd{MCF each designed, conducted, analyzed, and wrote up one of the rescue projects -- these authors contributed to investigation, formal analysis, and visualization.
}

\DIFadd{VB wrote the first draft, BP and MCF contributed editing, and all authors reviewed the final manuscript}\DIFaddend .
...
\DIFaddbegin \bibitem[\citeproctext]{ref-chica2009}
\DIFadd{Chica, A. B., \& Christie, J. (2009). Spatial attention does improve temporal discrimination. }\emph{\DIFadd{Attention, Perception, \& Psychophysics}}\DIFadd{, }\emph{71}(2)\DIFadd{, 273--280.
}

\DIFaddend \bibitem[\citeproctext]{ref-child2018}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-ngo2021}
\DIFadd{Ngo, C. T., \& Newcombe, N. S. (2021). Relational binding and holistic retrieval in ageing. }\emph{\DIFadd{Memory}}\DIFadd{, }\emph{29}(9)\DIFadd{, 1197--1205. }\url{https://doi.org/10.1080/09658211.2021.1974047}

\DIFaddend \bibitem[\citeproctext]{ref-olsson-collentine2020}
...
\DIFdelbegin \bibitem[\citeproctext]{ref-openscienceconsortium}
\DIFdel{Open Science Consortium}\DIFdelend \DIFaddbegin \bibitem[\citeproctext]{ref-opensciencecollaboration}
