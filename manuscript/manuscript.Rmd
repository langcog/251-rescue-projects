...
Replications can fail for any reason. In principle, the question of why a replication failed is the same as asking why did two closely related studies have different results, which could be due to any differences between the study. In the case of replication failure, there's an asymmetry where the original study happened first and set the methods and analyses and got the more significant result.  [VB is struggling to articulate ideas here] 
One source of replication failure is that the result is fragile in some way: it could be a statistically unlikely result that was achieved through chance or p-hacking, or it could be sensitive to the exact conditions and time it was run under ("hidden moderators"). The reported effect sizes might be biased estimates due to some combination of heterogeneity, p-hacking, luck, underpowering, and publication bias. The replication study could be underpowered to detect the effect, perhaps due to powering for the reported (inflated) effect size, or unexpected attrition in the replication sample. 

Replications might get different results from an original study due to differences in how the question of interest was instantiated; these differences in design could be intentional adaptations, or they could be unintentional if a replication recreates instructions or materials because the original ones are not available, or they could be trivial differences that turn out to be crucial to the effect. 

A difference in experiment platforms (ex. online versus in-person) might mean that an implementation doesn't fit the platform, or the data quality controls and attention checks are inadequate. If there are differences in time, place, or subject population between the two studies, there may need to be differences in the materials, instructions, or procedure to provide a good *translation* -- too many changes could cause differing results, but so could not enough changes. 

When replications fail, we generally do not know why they failed, although we often speculate on potential reasons. [TODO not sure how to transition here] The goal of most large-scale replication studies has been metascientific, with the aims of estimating the true proportion of effects in a literature or understanding the role of heterogeneity. For these purposes, it may not matter exactly why any individual replication failed because they are assessed as a whole. 

But when individual scientists attempt to replicate an effect with the goal of building on it, they do care about the individual failed replication. In an uncertain literature, we might not want to commit resources to building on an effect without first checking that we, with our resources and participant sample, can get the core effect we wish to build on. Thus, it's very important whether the replication failure was caused by something fixable about the replication or whether it was caused by something not-fixable about the original result (ex. extreme lack of generalizability, p-hacking, etc). 

While one can theorize after the fact about potential causes of replication failure, in most circumstances it is hard to diagnose for certain what made for the difference in the results. An exception might be if a specific error in one study or the other is found, such that the results were not what was thought (analytic error) or the data was not what was thought (experimental error). Thus, we usually can't know whether the replication failure was caused by factors that could be feasibly addressed or not. 

Re-replication could serve as a way of triangulating on reasons for replication failure, as well as another chance for a scientist to get a working protocol that picks up the effect of interest. Thus, scientists may want to know both how often and when a re-replication is likely to work and detect an effect that is indicative that future experiments in this tradition are likely to be informative.




<!--TODO THIS PARAGRAPH NEEDS HELP Factors that suggest another failure include heterogeneity and lack-of-generalizability ("hidden moderator"), as the second replication is likely to be closer to the first replication than to the original in these random factors.
...
If a second replication can get closer to the original, either by using closer materials, or better adapting the study to a new environment, the second replication might succeed at better approximating the results of the original.
A failed manipulation check failure may not be fixable, but it might at least be diagnoseable.-->


Re-replication has not been studied much, so it is uncertain how definitive a replication failure is. 
One study on re-replications was @bersole2020 which re-replicated 10 psychology studies that had failed replications reported in RP:P [@openscienceconsortium]. 
After  @openscienceconsortium published their results with a replication rate around 40%, @gilbert2016 raised concerns that methodological differences were the reason for the low replication rate. In 11 of the OSF studies, concerns about protocol fidelity had been raised by original authors prior to data-collection (and thus not contingent on results).
...
Prediction intervals depend on both the original effect size and variance CITATION TODO, and also the variance of the comparison (replication or rescue) study.
...
@tarampi2016 original had participants indicate their answers (left or right) on a piece of paper in a timed navigation task, the replication had them indicate by clicking a drop down, and rescue went with pressing keyboard keys to indicate the direction of the a turn.
TODO others to note?
...
As a compliment to the subjective rating of overall success, we stastically compared one key measure of interest for each study, following @boyce2023. In order to statistically compare the key measures, we needed effect sizes reported in the same way for each original study, first replication, and rescue.
...
We also recorded the same set of potential correlates that were used in @boyce2023 for original, first replication, and rescue (these were already rated for original and first replication).
...
[TODO Ben: I think the paragraph below could use more explanation about what $p$-original is, but I'm not confident enough in my understanding of it to write it myself.]
...
We also added some (non-preregistered) predictors related to the sample size of the first replication, after seeing the successful re-replications of @ngo2019 and @krauss2003, both studies with small replication samples.
...
Especially if there is not a clear, identifiable reason for the first replication's failure, another try is unlikely to recover the original result.
